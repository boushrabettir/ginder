...
def request_identity(client_id: str, redirect_uri: str, login: str, scope: str, state: str, allow_signup: str):
  data = {
    'client_id': client_id,
  ...
  authorize_app = requests.post(f'{GITHUB_ENDPOINT}/oauth/authorize', data=data) # The request to authorize the app, data=data is the data that is sent to the api
  return authorize_app.json() # Returns the authorization code 

# Gets the access token
def redirect_to_github(client_id: str, client_secret: str, code: str, redirect_uri: str):
  data = {
  ...
  access_token_request = requests.post(f'{GITHUB_ENDPOINT}/oauth/access_token', data=data) # The request to get the access token
  return access_token_request.json() # Returns the access token which is used to make requests to the api
...
def verify_user(client_id: str, scope: str, device_code: str, user_code: str, verification_uri: str, expires_in: int, interval: int):
  data = {
    'client_id': client_id,
    'scope': scope,
  ...
  verification_code = requests.post(f'{GITHUB_ENDPOINT}/login/device/code', data=data) # The request is to get the verification code
...
def check_authorization(client_id: str, device_code: str, grant_type: str):
  ...
  authorization = requests.post(f'{GITHUB_ENDPOINT}/login/oauth/access_token', data=data) # The request to check if the user has authorized the app
